...
export const AddSport = catchAsync(async (req, res) => {
  ...
  const newSport = new sportModel(data);

  // Check if files were uploaded
  const files = req.files;
  if (files && Object.keys(files).length > 0) {
    if (files.banner_image) newSport.banner_image = files.banner_image[0].filename;
    if (files.images) {
      let allImages = files?.images.map((image) => image.filename);
      newSport.images = allImages;
    }
    // if(files.videos){
    //     let allImages = files?.images.map((image) => image.filename);
    //     newSport.images = allImages;
...
export const UpdateSport = catchAsync(async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(sportId)) {
    return res.status(400).json({ status: false, message: "Invalid sport ID", data: null });
  }
  ...
  const sport = await sportModel.findByIdAndUpdate(sportId, { $set: updateData }, { new: true, runValidators: true });
  if (!sport) {
  ...
  if (search) {
    ...
    searchQuery = {
      ...
      $or: [
        ...
        // { capacity: isNaN(parseInt(search)) ? null : { $lte: parseInt(search) } },
        ...(isNaN(parseInt(search))
          ? [] // If not a valid number, don't include rate conditions
...
export const UpdateImages = catchAsync(async (req, res) => {
  ...
  updateData.images = existingSport.images;
  ...
  if (deletedImages) {
    ...
    if (Array.isArray(deletedImagesArray) && deletedImagesArray.length > 0) {
      if (updateData.images && Array.isArray(updateData.images) && updateData.images.length > 0) {
        ...
        let deleteData = await deleteImagesFromStorage("uploads/sport", deletedImagesArray);
        console.log(deleteData);
      } else {
        return res.status(400).json({
          status: false,
          message: "There is no images in this sport to delete.",
          data: null,
        });
      }
    }
